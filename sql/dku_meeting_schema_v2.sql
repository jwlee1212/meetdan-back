-- ============================================================
--  ⛔⛔⛔  멈추세요. 이 파일은 데이터베이스를 전부 지웁니다.  ⛔⛔⛔
--
--  아래 0번 절이 테이블 11개를 drop 합니다.
--
--      profiles      teams        team_members   matches
--      chat_rooms    messages     blocks         reports
--      admins        banned_words posts
--
--  실행하는 순간 사라지는 것 — 되돌릴 수 없습니다:
--      · 가입한 회원의 프로필 전부
--      · 만들어진 팀과 팀원 전부
--      · 신청·매칭·채팅 기록 전부
--
--  auth.users(계정과 비밀번호)만 auth 스키마라 살아남습니다. 그래서
--  "로그인은 되는데 앱이 통째로 비어 있는" 상태가 아니라, 프로필이 없어
--  **로그인 자체가 막히는** 상태가 됩니다.
--     → 그때는 dku_meeting_backfill_profiles.sql 로 프로필만 되살릴 수
--       있습니다. 팀·매칭·채팅은 사본이 없어 복구 불가입니다.
--
--  ── 이 파일을 돌려도 되는 경우는 하나뿐입니다 ──
--      비어 있는 새 프로젝트에 스키마를 처음 세울 때.
--
--  ── 스키마를 고치고 싶다면 ──
--      이 파일을 고쳐서 다시 돌리지 마세요.
--      dku_meeting_migration_012_*.sql 을 새로 쓰세요.
--      (001~011 이 그렇게 쌓여 있고, 그 파일들은 데이터를 지우지 않습니다)
--
--  ⚠ 다시 돌렸다면 마이그레이션 002~011 을 순서대로 전부 재적용해야
--    합니다. 여기서 만드는 profiles 에는 birth_year 가 없고(006 이 추가),
--    가드 트리거와 정책도 테이블과 함께 사라진 상태입니다.
-- ============================================================
...
-- 0. 기존 객체 정리
--
--    ⛔ '정리'라고 썼지만 실제로는 데이터 전체 삭제다. cascade 라 테이블에
--       딸린 행·정책·트리거·외래키가 전부 함께 사라진다. 되돌릴 수 없다.
--       이 파일 맨 위 경고를 읽지 않았다면 지금 멈출 것.
-- ============================================================
